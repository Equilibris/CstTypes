...
theorem cons_append {A B hd} {a : TyArr A} {b : TyArr B} {z : ArgCount hd}
...
def get
    {Γ ty}
    {idx : Nat} (h : Γ[idx]? = some ty) (v : TyArr Γ)
...
theorem get_append_right
  ...
  | [], .nil, idx => by
    ...
    rfl
  | hd :: tl, .cons hd' tl', 0 => by simp at hLe
  | hd :: tl, .cons hd' tl', n+1 => by
    change (hd :: (tl ++ _))[n + 1]? = _ at h
    change get _ (TyArr.cons hd' (tl' ++ Γ₂')) = _
    dsimp [get]
    simp only [List.length_cons, Nat.add_le_add_iff_right] at hLe
    dsimp at h'
...
theorem Every_concat {A B p} {a : TyArr A} {b : TyArr B} (h1 : Every p a) (h2 : Every p b) : Every p (a ++ b) :=
  match h1 with
  | .nil => h2
