...
def Term'.show {ty}
    (expr : @Term' String (fun _ => String) ty) (level : Nat := 0) (typelevel : Nat := 0) : String :=
  match expr with
  | .var s => s
  ...
  | @Term'.tapp _ _ f term1 t =>
    s!"({Term'.show term1 level typelevel} [{Type'.show (Type'.squash <| f t) typelevel}])"
  | @Term'.tlam _ _ f g =>
    s!"(Λ {natToGreek typelevel} . {Term'.show (g <| .var (natToGreek typelevel)) level (typelevel + 1)})"
