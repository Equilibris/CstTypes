...
import Types.STLC.Red.Terminal
...
inductive TySpec : List Ty → Stx → Ty → Prop
  ...
  | app {Γ fn argTy retTy arg} : TySpec Γ fn (argTy ⇒ retTy) → TySpec Γ arg argTy
      → TySpec Γ (.app fn arg) retTy
  | abs {Γ body ty retTy} : TySpec (ty :: Γ) body retTy
...
theorem TySpec_abs {Γ argTy body retTy}
    : TySpec Γ (.abs argTy body) (argTy ⇒ retTy) ↔ (TySpec (argTy :: Γ) body retTy) := by
  constructor
  · rintro (_|_|a)
    exact a
  · exact (.abs ·)
...
theorem TySpec_abs' {Γ argTy body z}
    ...
    ↔ ∃ retTy, (z = (argTy ⇒ retTy)) ∧ (TySpec (argTy :: Γ) body retTy) := by
  constructor
  · rintro (_|_|a)
    rename_i retTy
    exists retTy
...
theorem Terminal_iff_non_Red
    {Γ ty a}
    (typed : TySpec Γ a ty) : (∀ b, ¬Red a b) ↔ Terminal a
...
theorem TypePreservation
  ...
  match h, hTy with
  | .appl h, .app ha hb => .app (TypePreservation h ha) hb
  | .appr h, .app ha hb => .app ha (TypePreservation h hb)
  | .congr h, .abs hTy  => .abs <| TypePreservation h hTy
  | .beta, hTy          => TySpec_replace ‹_›

theorem Progress
    {Γ stx ty}
    (hTy : TySpec Γ stx ty) : Terminal stx ∨ ∃ stx', Red stx stx' :=
  match stx with
...
theorem LongTypePreservation
    {Γ ty e e'}
    (spec : TySpec Γ e ty) (h : RedStar e e') : TySpec Γ e' ty := by
  induction h
  · exact spec
...
theorem Safety
    {Γ ty e e₁}
    (spec : TySpec Γ e ty) (h : RedStar e e₁) : (Terminal e₁ ∨ ∃ e₂, Red e₁ e₂) := by
  induction h
  · exact Progress spec
...
theorem replace_gt_Γ
    {Γ s t idx repl}
    (h : TySpec Γ s t) (hLe : Γ.length ≤ idx) : s.replace idx repl = s :=
  match s with
  | .bvar jdx => by
    cases h; next h =>
    have hLt : jdx < Γ.length := by grind
